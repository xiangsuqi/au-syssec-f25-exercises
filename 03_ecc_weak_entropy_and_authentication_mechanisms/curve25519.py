...
def point_addition(P, Q):
    ...
    if x1 == x2:
        if y1 == y2:
            return point_doubling(P)  # P == Q, use point doubling
        else:
            return None  # Point at infinity

    # Calculate the slope lambda
    lambda_ = (y2 - y1) * pow(x2 - x1, p - 2, p) % p
...
def point_doubling(P):
    ...
    # Calculate the slope lambda
    lambda_ = (3 * x1 * x1 + a) * pow(2 * y1, p - 2, p) % p

    # Calculate x3 and y3
    x3 = (lambda_ * lambda_ - 2 * x1) % p
    y3 = (lambda_ * (x1 - x3) - y1) % p
...
def montgomery_ladder(k, P):
    ...
    for i in range(k.bit_length() - 1, -1, -1):
        if (k >> i) & 1:
            R0 = point_addition(R0, R1)
            R1 = point_doubling(R1)
        else:
            R1 = point_addition(R0, R1)
            R0 = point_doubling(R0)
...
assert scalar_mult(sk_alice, PK_bob) == montgomery_ladder(
    sk_alice, PK_bob
), "Montgomery ladder is wrong"
assert scalar_mult(sk_bob, PK_alice) == montgomery_ladder(
    sk_bob, PK_alice
), "Montgomery ladder is wrong"
